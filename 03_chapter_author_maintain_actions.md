...
- **Publishing to the GitHub Marketplace** requires: the action's `action.yml` to live at the **repository root**, a semantic-version **release tag** (Marketplace lists actions by release, and prompts you to also move/create the matching major-version tag like `v1` at publish time), and a `branding:` block (an `icon` from the Feather icon set plus a `color`) so the listing has a visual identity instead of a generic default. A repo can only publish **one** action to the Marketplace per top-level `action.yml` — bundling multiple actions in one repo (common for a suite of related actions) means each one needs its own subdirectory with its own `action.yml`, and only the root-level one is Marketplace-eligible directly; the others are consumed via `uses: owner/repo/subdir@ref` without a Marketplace listing of their own.
- Marketplace publishing is a **visibility and discoverability** mechanism, not a trust or security signal — anyone can publish, and the presence of a Marketplace listing says nothing about whether an action's code is safe to run. Pinning discipline (above) applies identically whether an action came from the Marketplace or a random public repo.
